#Shadman Choudhury, Nadia Lara, Derrick Sual - Project 3 - GEDCOM Parsing Group Project
    def __init__(self, line, parent):
        ...
        if(len(components) > 2):
            if(components[2] in ["INDI", "FAM"]): #specific case with INDI and FAM, the tags are arg 2
                tag = components[2]
                arguments = components[1]
                valid = level == 0 and parent == None #valid location and argument, true if level is 0 and parent is None
                if(len(components) > 3): #too many arguments, invalid
                    arguments += ' '.join(components[3:])
                    valid = False
            elif(components[1] == "INDI" or components[1] == "FAM"): #right tag, wrong arg for edge cases
                tag = components[1]
    ...
    def _validate(self):
        ...
        if(len(components) > 0):
            if(self.tag == "NOTE"): #any string works, valid if the level/parent is correct
                self.valid = self.level == 0 and self.parent == None
            elif(self.tag == "NAME"): #any string works, valid if level is 1, may not have surname, must be part of INDI
                self.valid = self.level == 1 and self.parent and self.parent.tag == "INDI"
            else: #Name, Note, Indi, Fam taken care of
                if(self.tag == "DATE"):
                    if(self.parent and self.parent.tag not in ["BIRT", "DEAT", "DIV", "MARR"] or self.level != 2):
                        self.valid = False #wrong parent or wrong level
                    else:
                        if(len(components) == 3 and 0 < int(components[0]) < 32 and components[1] in ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"] and 999 < int(components[2]) < 10000):
                            self.valid = True #exact number of args for DATE, with the correct formatting
                        else:
                            self.valid = False
                elif(self.tag in ["FAMC", "FAMS"]):
                    if(self.parent and self.parent.tag == "INDI" and len(components) == 1): #prereqs
                        self.valid = self.level == 1 #valid if level is 1
                    else:
                        self.valid = False #too many args/wrong parent
                elif(self.tag == "SEX"):
                    if(self.parent and self.parent.tag == "INDI" and len(components) == 1):
                        self.valid = self.level == 1 and components[0] in ["M", "F"] #valid if correct level and specific argument
                    else:
                        self.valid = False #too many arguments or wrong parent
                elif(self.tag in ["HUSB", "WIFE", "CHIL"]):
                    if(self.parent and self.parent.tag == "FAM" and len(components) == 1):
                        self.valid = self.level == 1 #valid if level is 1
                    else:
                        self.valid = False #too many args, wrong parent, or wrong level
                else:
            ...
            elif(self.tag in ["DIV", "MARR"] and self.parent and self.parent.tag == "FAM" and self.level == 1):
                self.valid = True #parent is correct and level is correct
            elif(self.tag in ["DEAT", "BIRT"] and self.parent and self.parent.tag == "INDI" and self.level == 1):
                self.valid = True #parent is correct and level is correct
            else:
